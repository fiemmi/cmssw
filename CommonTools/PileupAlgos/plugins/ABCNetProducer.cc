...
private:
  ...
  void preprocess(std::unordered_map<std::string, std::vector<float>> & features);
  // tokens
...
void ABCNetProducer::preprocess(std::unordered_map<std::string, std::vector<float>> & features) {
  for (auto & element : features) { //do not declare 'auto const', want to manipulate the element
